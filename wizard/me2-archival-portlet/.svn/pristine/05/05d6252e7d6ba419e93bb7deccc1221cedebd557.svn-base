...
public class ArchivalMonitorLoggerUtil
{
    ...
    //private static String LOGGER_PATH="/apps/merlin/log4j/Indexer/merlin-ndexer.log";
     ...
     * Default constructor.
     */
    private ArchivalMonitorLoggerUtil()
    {
    }
     ...
     * @return logger
     */
    public static synchronized Logger getLogger(String name, org.apache.log4j.Level level)
    {
        ...
        if (logger == null)
        {
            ...
            if (appender == null)
            {
                try
                {
                    appender = new RollingFileAppender(new PatternLayout("%-5p %c.%M[%L] %d - %m%n"),
                            LOGGER_PATH, true);
